...
Because RC4 is a stream cipher any missing data would cause a misalignment with the key-stream. Because in wifi frames are expected to be lost the process is restarted each frame.  To prevent key-stream reuse, a random 24-bit Initialization Vector (IV) is appended to the shared key.
